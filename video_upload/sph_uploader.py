...
import upload.tencent_uploader.main as TencentVideo
...
def download_video(url, output_filename):
    try:
        ...
        loguru.logger.info(f"下载视频时发生错误: {e}")
...
def scheduled_job():
    ...
    for shipinhao_info in shipinhao_user_info:
        ...
        if pub_gap < 3600:
            ...
            loguru.logger.info(f'账号{shipinhao_info.shipinhao_username}上一次的视频发布时间为{latest_pub_time},间隔低于一小时，等待后续再发')
            continue
        ...
        if pubbed_num >= pub_num:
            loguru.logger.info('视频号账号{}, 发布了超过{}个视频，暂停发布。'.format(user_id, len(published_videos)))
            continue
        ...
        if not cookie_valid:
            loguru.logger.info('用户{}的cookie已经过期'.format(shipinhao_info.shipinhao_username))
            continue
        ...
        if not deduplicated_videos:
            # 没有待发布的视频，则继续等待
            loguru.logger.info(f'账号{shipinhao_info.shipinhao_username}最近十分钟内没有去重完毕待发布的视频，继续等待')
            continue
